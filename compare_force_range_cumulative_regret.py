"""
Compare cumulative regret across FORCE RANGES for continuous constraints
(actuator_count = 8).

One panel per force range; each panel overlays the methods (Q-Safe BO,
C-Safe BO, BO-ACL). Mirrors compare_actuator_count_cumulative_regret.py.

Force-range → results-root mapping:
  200  lb -> Experiments_constraint_continuous_force_200
  500  lb -> Experiments_constraint_continuous_force_500
  1000 lb -> Experiments_constraint_continuous          (baseline)

Example:
  python compare_force_range_cumulative_regret.py
  python compare_force_range_cumulative_regret.py --forces 200 500 1000
"""
...
DEFAULT_ROOTS = {
    200: Path("Experiments_constraint_continuous_force_200"),
    500: Path("Experiments_constraint_continuous_force_500"),
    1000: Path("Experiments_constraint_continuous"),
}

METHOD_ORDER = ["Q-Safe BO", "C-Safe BO", "BO-ACL"]
METHOD_STYLE = {
    "Q-Safe BO": {"color": "#1f77b4", "marker": "o"},
    "C-Safe BO": {"color": "#2ca02c", "marker": "s"},
    "BO-ACL":    {"color": "#d62728", "marker": "^"},
}
FORCE_STYLE = {
    200:  {"color": "#1f77b4", "marker": "o"},
    500:  {"color": "#2ca02c", "marker": "s"},
    1000: {"color": "#d62728", "marker": "^"},
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare cumulative regret across force ranges (actuator_count=8)."
    )
    parser.add_argument(
        "--forces",
        type=int,
        ...
        help="Force ranges (lb) to compare. Defaults to 200 500 1000.",
    )
    parser.add_argument(
        "--noise",
        type=float,
...
def discover_runs(
    ...
    for force in forces:
        root = roots[force]
        if not root.exists():
            print(f"[WARN] force={force}: missing root {root}")
            continue
...
def plot_results(
    ...
    present_methods = [
        method for method in methods if any((method, force) in stats for force in forces)
    ]
    ...
    for idx, (ax, method) in enumerate(zip(axes[0, :], present_methods)):
        for force in forces:
            ...
            style = FORCE_STYLE.get(force, {"color": None, "marker": None})
    ...
    fig.suptitle(
        f"Cumulative Regret by Force Range (actuator_count=8), $\\sigma$ = {np.sqrt(noise):g}",
        fontsize=14,
    )
...
def main() -> None:
    ...
    for row in rows:
        grouped_files[(row["method"], row["force"])].append(row)
    ...
    for method in methods:
        for force in args.forces:
            group = grouped_files.get((method, force), [])
            if not group:
                print(f"  {method:16s} force={force}: NO DATA")
                continue
