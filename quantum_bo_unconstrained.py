"""
Quantum Unconstrained Bayesian Optimization — W-GP-UCB (Continuous, Multi-GPU)
=============================================================================
Unconstrained baseline aligned to quantum_safeset_continuous.py so the two can
be compared apples-to-apples (the quantum analog of
classic_bo_unconstrained.py ↔ classic_safeset_continuous.py). Every initial
setting is identical:

  - Same environment (quantum_bo_env_constraint.QuantumFuselageEnv) and objective
  - Same ProcessPoolExecutor multi-GPU model (one trial per GPU via tri % count)
...
The ONLY difference vs. the safe-set script is the acquisition:
  ...
  - Selection is pure W-GP-UCB argmax over the FULL candidate pool.
  (The QAE constraint value c_val is still returned per query, but ONLY to
   report the violation rate of this baseline.)
"""
import argparse
...
def save_data(actions, true_response, response, num_of_queries, eps, error_init, file_path):
    """Signature matches quantum_safeset_continuous.py save_data exactly."""
    torch.save({
        'actions': actions,
...
def W_GP_UCB_scores(model, x_D, response, V_t, Unweighted_Phi,
    ...
    """
    Memory-safe W-GP-UCB scoring (identical to quantum_safeset_continuous.py).
...
def run_trial(tri, seed, args, input_filename, target_filename,
    ...
    # ── Assign GPU (matches quantum_safeset_continuous.py) ────────────────
    ...
    try:
        ...
        # ── BO loop (unconstrained: pure W-GP-UCB over the full pool) ─────
        while num_of_queries_num < n_iter:
            ...
            # c_val is evaluated ONLY to report the violation rate of this baseline.
            ...
            if stage_count % 100 == 0 or num_of_queries_num >= n_iter:
                save_data(
                    ...
                    error_init,
                    path.join(log_dir,
                              str(obs_noise) + 'quan_unconstrained_training_data_' + str(tri) + '_.pth'),
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Multi-GPU Quantum Continuous Unconstrained W-GP-UCB (aligned to safe-set baseline).")
    ...
    lam        = args.lam
    Big_B      = args.B
    print("Multi-GPU Quantum Continuous Unconstrained W-GP-UCB (ProcessPoolExecutor)")
    ...
    for seed in range(args.outer_seeds):
        ...
        env_name = "Quantum_Unconstrained_{}_{}_{}_{}_multi_gpu_noise_{}".format(
