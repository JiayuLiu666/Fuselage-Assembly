...
class QuantumFuselageEnv(object):
    def __init__(self, obs_noise, ip=None, n_actuators=10, backend=None, min_shots=100, force_scale=1000.0):
        self.ip = ip
    ...
    def reset(self, filepath, seed=None, target_npy=None):
        ...
        # np.random.seed(0)
        # torch.manual_seed(0)
        self.forces = np.zeros(18, dtype=np.float64) 
        ...
        folder = path.join(__file__, 'Shapes', 'Test') 
        # Target shape: use target_npy if provided, otherwise default
        ...
        self.targetPos = np.load(filepath).astype(np.float64)
        ...
        return torch.tensor(self.error_init, dtype=torch.float64).reshape(1, 1), file2
    ...
    def cal_deviation(self, action):
        ...
        self.forces = np.array(self.forces, dtype=np.float64)
    ...
    def step_tsai_wu(self, action, device=None):

        action = action.reshape(-1)

        FI = self.tsai_wu.predict((action).reshape(1,-1), return_std=False)
    ...
    def step_surrogate(self, action, eps, device=None):
        ...
        # dynamic Chernoff-Hoeffding confidence bound formula
        ...
        if hasattr(self, 'backend') and self.backend is not None:
            estimation, scale_obs, oracle_queries_complexity, num_oracle_queries, conf_int = iterative_amplitude_estimation(
                problem=problem, 
            ...
            empirical_variance = (((conf_int[1] - conf_int[0])) / self.error_init) ** 2

            if num_oracle_queries == 0:
                num_oracle_queries = int(np.ceil((0.8 / epsilon) * np.log((2 / alpha) * np.log2(np.pi / (4 * epsilon))))/2)
            ...
            empirical_variance = (((conf_int[1] - conf_int[0]))) ** 2
        
            if num_oracle_queries == 0:
                num_oracle_queries = int(np.ceil((0.8 / epsilon) * np.log((2 / alpha) * np.log2(np.pi / (4 * epsilon))))/2)
            
        total_gp_variance = empirical_variance 

        return torch.tensor(scale_obs, dtype=torch.float64).reshape(1,1).to(device), \
                torch.tensor(self.error, dtype=torch.float64).reshape(1,1).to(device), \
                torch.tensor(num_oracle_queries).reshape(1,1).to(device), \
                torch.tensor(c_val, dtype=torch.float64).reshape(1,1).to(device), \
                torch.tensor(total_gp_variance, dtype=torch.float64).reshape(1,1).to(device)
