"""
Classic Constrained Bayesian Optimization — Safe-Set cUCB (Continuous)
========================================================================
Classical counterpart of quantum_bo_continuous_constrain_multi_gpu.py.

Parallelism model (matches classic_bo_unconstrained.py):
...
Algorithm aligned with quantum multi-GPU:
  - Same LHS warmup + vectorised MAE, with noisy warmup observations
...
def lambda_by_stage(stage, lam0=1, t0=400, p=2.0):
    """Matches quantum_bo_continuous_constrain_multi_gpu.py."""
...
def save_data(actions, true_response, response, num_of_queries, eps, error_init, file_path):
    """Signature matches quantum save_data exactly."""
    torch.save({
        'actions':       actions,
...
def fit_c_model(actions, response, device):
    """Fit constraint GP once; hyperparameters frozen afterwards (matches quantum)."""
...
def compute_lcb_c_grad(c_model, X, beta_c: float):
...
def run_trial(tri, seed, args, input_filename, target_filename,
    ...
    # ── Assign GPU (matches classic_bo_unconstrained.py) ─────────────────
    gpu_id = tri % torch.cuda.device_count() if torch.cuda.is_available() else 0
    device = torch.device(f'cuda:{gpu_id}' if torch.cuda.is_available() else 'cpu')

    ckpt_path = path.join(
        log_dir,
        f"{obs_noise}classic_safeset_ckpt_{tri}_.pth")
    ...
    try:
        ...
        # ── LHS warmup (same seed as quantum) ────────────────────────────
        ...
        FI_batch          = tsai_wu_model.predict(warmup_np, return_std=False)
        constraint_warmup = (1 - FI_batch).tolist()

        # Test set for GP hyperparameter fitting
        ...
        c_Y = torch.tensor(constraint_warmup, dtype=torch.double,
        ...
        # ── Constraint & Objective GPs on warmup data ONLY ───────────────
        c_model = fit_c_model(c_X, c_Y, device)
        ...
        if init_c_margin < 0.0:
            raise RuntimeError(
                f"Initial action is outside the safe set: margin={init_c_margin:.6g}"
            )
        init_c_val = torch.tensor(
            [[init_c_margin]],
        ...
        #         device, M_target,
        #         ls_init=lengthscale, outputscale=outputscale,
        #         obs_noise=objective_yvar,
        #     )
        #     f_model.eval()
        ...
        while num_of_queries_num < n_iter:
            ...
            mu_c, sig_c, lcb_c, _ = compute_lcb_c(c_model, candidate_actions, beta_c)
            safe_mask = lcb_c >= 0.0
            ...
            if safe_mask.sum().item() > 0:
                ...
                ucb_min = ucb_f[safe_mask].min().item()
                ucb_max = ucb_f[safe_mask].max().item() + 1e-9
                bnd_min = a_bnd[safe_mask].min().item()
                bnd_max = a_bnd[safe_mask].max().item() + 1e-9
                ...
                for _ in range(50):
                    ...
                    mu_c_opt, sig_c_opt, lcb_c_opt, _ = compute_lcb_c_grad(
                ...
                with torch.no_grad():
                    ...
                    mu_c_final, sig_c_final, final_lcb_c, _ = compute_lcb_c_grad(
            ...
            c_val       = torch.tensor([[c_val_float]], dtype=torch.double, device=device)
            ...
            # Fantasy model O(1) constraint update
            ...
            if stage_count % 100 == 0 or num_of_queries_num >= n_iter:
                save_data(
                    ...
                    error_init,
                    path.join(log_dir,
                              str(obs_noise) + 'classic_safeset_training_data_' + str(tri) + '_.pth'),
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Classic Continuous Safe-Set cUCB.")
    ...
    print("Classic Continuous Safe-Set cUCB")
    ...
    for seed in range(args.outer_seeds):
        ...
        env_name = "Classic_SafeSet_{}_{}_{}_{}_noise_{}".format(
