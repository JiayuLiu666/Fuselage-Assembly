...
def initialize_model(actions, response, device, M_target,
                     obs_noise=NOISE_FREE_VARIANCE, ls_init=None):
...
if __name__ == "__main__":
    ...
    print("Quantum Discrete GP-UCB (Real Hardware), 2 actuators")
    ...
    env_name = "Quantum_Discrete_cUCB_Real"
    ...
    print("\n[Quantum Discrete CBO (Real)] Connecting to IBM Quantum...")
    service_ibm = QiskitRuntimeService()
    try:
        real_backend = service_ibm.least_busy(operational=True, simulator=False, min_num_qubits=127)
        print(f"[Quantum Discrete CBO (Real)] Selected backend: {real_backend.name}")
    except Exception as e:
        print(f"[Quantum Discrete CBO (Real)] Failed to get backend: {e}")
        real_backend = None
    ...
    for tri in trials:
        ...
        env = QuantumFuselageEnv(obs_noise=obs_noise, min_shots=min_shots, backend=real_backend) 
        ...
        # Best combo: B=0.5, active_LS=0.20 — 5/5 hit rate, avg 50 iters (for obs_noise=0.04)
        # For obs_noise=0.01, use active_LS=0.50
