...
class ClassicFuselageEnv(object):
    ...
    def reset(self, filepath, seed, target_npy):
        random.seed(seed)
        # np.random.seed(0)
        # torch.manual_seed(0)
        ...
        folder = path.join(__file__, 'Shapes', 'Test') 
        # Target shape: use target_filename if provided, otherwise default
    ...
    def step_surrogate(self, action, method=None, device=None, eps=None):
        ...
        relative_obs, num_oracle_queries = self._monte_carlo_estimate(draw=lambda: self.draw_gaussian(mean=-self.error, std=stddev), var=variance, \
            epsilon=eps, delta=0.05, method=method, max_samples=4999)     
